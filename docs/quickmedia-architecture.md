...
| 转码 | **v1 不内建**。留 `TranscoderBackend` SPI + 外部 FFmpeg worker（探测 NVENC/QSV/VAAPI 并降级）。理由见 §7.6。 |
...
**最需要评审方拍板的两件事**（详见 §9）：
1. 国标安防（GB28181 + 对讲 + 级联）到底进 P1 还是作为独立商业发行版——它决定 SIP 栈选型与 M3 的风险承担方式。
2. 是否承诺"内建转码"为长期能力——调研报告已确认这是两家开源基线共同缺口，一旦承诺就要承担闭源厂商级的工程投入。
...
| 非目标 | 理由 |
|---|---|
| 内建转码（v1） | 两家基线共同缺口；工程投入与"不改内核"冲突（§7.6） |
...
| C SDK / 嵌入模式 | v1 不做；Go 无法提供 C ABI 嵌入。若这是硬需求，直接推翻 §7.1（成本见该节） |
...
**v1 结论：内核不做编解码，因此不需要硬解。** 转码只在外部 worker 中发生，硬加速是 worker 的内部问题：

```
TranscoderBackend SPI（内核侧，v1 只定义不实现）
   ├── v1: FFmpegWorker —— 探测 NVENC > QSV > VAAPI > libx264，按 GPU 可用性降级
   │        降级链在 worker 启动时确定并上报能力，运行中不可切换（避免中途重建管线）
   └── v2+: NvTranscodeBackend / QSV in-proc（仅在"内建转码"被确认为长期目标后启动）
```

**为什么不在内核里放 NVENC 直连**：这会立刻把 GPU 生命周期、显存池、会话并发上限、驱动崩溃恢复拉进内核热路径，与"内核纯净"（G5）冲突。外部 worker 让这些风险被隔离在进程边界之外——这也意味着转码故障不会拖垮媒体面。
...
| 协议/能力 | 补齐方式 | 备注 |
|---|---|---|
| GB/T 28181（SIP + PS + RTP，含双向对讲、主动拉流、级联） | 内建 Adapter + SIP 栈待定（pion 系 vs C 网关子进程） | **最大技术不确定项**，见 §9.1 |
...
| 内建转码 | v1 外部 FFmpeg；v2 视 §9.2 决策 | 见 §7.6 |
...
- **结论**：**分级**。RTSP/RTMP/SRT/HLS/WebRTC/MoQ 复用上游；单端口复用、连接迁移、国标、TS 透传、集群迁移、绝对时间戳自研。
- **放弃理由**：全自研的投入与"同一量级性能"目标相冲突（要超越已优化多年的栈）；全复用则拿不到差异化。
- **反悔成本**：低-中。上游锁定通过 `go.mod` 版本锁 + 定期 rebase 管理，风险是上游 API 漂移（需为每个上游库写适配隔离层）。

### 7.6 是否内建转码
- **选项**：v1 内建 / v1 外部 worker + v2 内建 / 永远不内建
- **结论**：**v1 外部 FFmpeg worker（探测 NVENC/QSV/VAAPI 降级），v2 视需求内建**。
- **放弃理由**：内建转码要把 GPU 生命周期、显存池、驱动崩溃恢复、编码参数协商全部拉进内核热路径；且这是两家基线共同的商业收费点，投入产出比需要产品决策支撑。
- **反悔成本**：**低**（SPI 已定义，`TranscoderBackend` 可直接实现）。**这是本设计中最安全的一次保守选择。**
- **需产品确认**：见 §9.2。
...
| 阶段 | 交付边界 | 验收证据（硬门槛） |
|---|---|---|
| **M0**（1 周） | 骨架 PoC：跑通 RTSP→RTMP→HLS→WebRTC 全链路；确立 Unit/Track/Stream 数据结构与注册表接口 | ① 四协议端到端视频可播；② `grep` 门禁脚本可在 CI 通过；③ 接口文档评审通过（本文 §2.2 冻结）；④ **Rust 生态复核记录**（1 天，非阻塞）：按 §7.1.1 的依赖清单核对 RTSP / enhanced-RTMP / WebRTC / MoQ 在 crates.io 的生产可用性，结论写入本文 §7.1.1 附注；触发 §9.3 的 T1–T4 任一条件则停止开发、重启语言决策 |
| **M1**（1–2 月） | P0 协议 7 项（RTSP/RTMP/HTTP-FLV/HLS/WebRTC/SRT/TS+RTP）+ Control API + Hooks + Prometheus | ① 性能基线 7 项实测数据（§4.5）全部产出并入库；② 单核 ≥1 万路 RTSP 只读（未达标则给出差距分析与是否降级目标）；③ 每 Unit 拷贝 = 0 的插桩证据；④ `bench` 子命令可复现 |
| **M2**（2 月） | + MoQ、fMP4 录像、绝对时间戳、always-available、点播+seek、JWT、TS 透传 | ① WebRTC 单跳 P95 < 300ms、LL-HLS < 1.5s 实测；② 录像回放与直播无缝切换演示；③ **易扩展演练**：新增一个玩具协议 + 1 个 codec + 1 个 capability，实测工时并写入本文 §5.1（内核 diff = 0 行） |
...
### 9.1 【需架构/产品确认】GB28181 的 SIP 栈选型
Go 生态无生产级 SIP。两条路：
- **(a) 自研 SIP over pion 式 UDP/TCP**：可控但实现量大（INVITE/BYE/REFER + SDP + PS 解封装 + 级联目录），M3 风险集中于此。
- **(b) C 网关子进程（opensips/kamailio）**：SIP 可靠但引入运维复杂度，且需走 §5.4 的 gateway 契约。
- **建议**：先用 (b) 做 M2 的 spike 验证对接可行性，再决定是否走 (a) 内建。**若最终决定内建，GB28181 应保留在 P1，不进入 P0。**

### 9.2 【需产品确认】是否把"内建转码"作为长期承诺
调研报告结论：内建转码是两家基线共同缺口，且是 ZLMediaKit 闭源版的收费点。一旦对外承诺，需承担 GPU 驱动、编码参数矩阵、显存治理的长期投入。**默认建议：不承诺**，v1 用外部 FFmpeg worker 覆盖 90% 场景，v2 视商业需求再投。

### 9.3 【需确认】C SDK / 嵌入模式是否为硬需求 —— 语言决策的首要未知项
若客户需要把 QuickMedia 嵌入自有应用（ZLMediaKit 的核心卖点之一），Go 无法提供 C ABI，必须重做内核（6–12 人月）。**这是最贵的未知项，建议在 M0 阶段就确认**，它同时是 §7.1.1 触发条件 T1——一旦确认需要，直接切 Rust（Rust 可导出稳定 C ABI），此时语言决策与 C SDK 需求同解，不需要第二次评审。

**语言决策的重启条件（§7.1.1，无需重新审批即可重启）**：T1 C SDK 为硬需求；T2 团队 3 个月内无法补齐 Go 主力；T3 硬抖动预算收紧到 GC 敏感量级；T4 Rust 协议栈出现生产级实现。**M0 必须完成一次 Rust 生态复核**（1 天，非阻塞）并记录到 §7.1.1，因为该节结论基于 2026-09 的知识状态，未经本轮实测复核。
...
│   ├── recorder/ playback/ snapshot/ forwarder/ transcoder/ storage/
